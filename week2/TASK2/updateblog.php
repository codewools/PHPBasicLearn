...
$sqlFetech = "SELECT * FROM `post` WHERE id = $id;";
$result = $conn->query($sqlFetech);
...
<body>
    ...
    <form action="writeblog.php" method="post" enctype="multipart/form-data">
            <input type="text" required name="title" value="<?php
            if (isset($row)) {
                echo $row['main_title'];
            }
            ...
            <textarea name="para1" required rows="10" cols="80"><?php
            if (isset($row)) {
                echo $row['para1'];
            }
            ?></textarea><br>
            ...
            <textarea name="hero_text" rows="10" cols="80">
